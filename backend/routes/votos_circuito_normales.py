...
votos_normales_bp = Blueprint("votos_normales", __name__)

@votos_normales_bp.route('/<int:num_mesa>/votos_normales/<int:id_eleccion>', methods=['GET'])
def get_votos_normales_solo(num_mesa, id_eleccion):
    ...
    try:
        ...
        # Obtener total de votos NORMALES (observado = 0)
        cursor.execute("""
            SELECT COUNT(*) as total_votos
            FROM voto v
            WHERE v.id_circuito = %s AND v.id_eleccion = %s AND v.observado = 0
        ...
        # Obtener votos anulados (NO observados)
        cursor.execute("""
            SELECT COUNT(*) as total_votos
            FROM voto v
        ...
        # Obtener votos en blanco (NO observados)
        cursor.execute("""
            SELECT COUNT(*) as total_votos
            FROM voto v
        ...
        if tipo_eleccion == 3:  # Plebiscito
            ...
            # TABLA 1: Lista - Partido - Cant. Votos - Porcentaje (SOLO NORMALES)
            cursor.execute("""
                SELECT 
            ...
            # TABLA 2: Partido - Votos - Porcentaje (SOLO NORMALES)
            cursor.execute("""
                SELECT 
                    p.nombre as partido,
                    COUNT(DISTINCT v.id) AS cant_votos
            ...
            # TABLA 3: Partido - Candidato - Cant Votos - Porcentaje (SOLO NORMALES)
            cursor.execute("""
                SELECT 
                    p.nombre as partido,
            ...
            # TABLA 4: Ganadores por Departamento (SOLO NORMALES)
            cursor.execute("""
                WITH votos_por_departamento AS (
                    ...
                    WHERE v.id_eleccion = %s AND v.observado = 0
